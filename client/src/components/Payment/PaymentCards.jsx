...
const PaymentCards = props => {
    ...
    <div className='payment-plan-choice' style={{'border': `${props.activePlan === data.productId ? '1px solid #ea9d9d' : ''}`}}>
        ...
        <button className='payment-product-button' disabled={props.activePlan === data.productId} style={{'backgroundColor' : `${props.activePlan === data.productId ? '#e76a6a' : ''}`}} onClick={() => {props.setActivePlan(data.productId); props.setPriceId(data.priceId)}}>{props.activePlan !== data.productId ? 'Selected' : 'Select'}</button>
    </div>
    )
